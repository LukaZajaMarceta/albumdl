...
re_get_time = re.compile(r'((\d{1,2})\D{1,2})?(\d{1,2})\D{1,2}(\d{2})')
...
class VideoLink:
    def __init__(self, url):
        ...
        self.description = self.video_api_response.snippet.description
        self.video_length = self.video_api_response.contentDetails.get_video_seconds_duration()
        self.comments = api.get_comment_threads(video_id=self.video_id,
                                                count=10,
                                                order='relevance',
                                                text_format='plainText').items
    ...
    def set_timestamps_comment(self, i):
        if i == 0:
            self.timestamps_comment = self.description
            return None
    ...
    def fitness_comment_timestamps(self, str):
        ...
        n_of_timestamps = len(re_get_time.findall(str))
        if n_of_timestamps < 2:
            return 10000
        else:
            song_length_assumption = 180
            return abs(self.video_length / n_of_timestamps - song_length_assumption)

    def gen_fit_comment(self):
        ct_by_video = self.comments
        comment_text = [x.snippet.topLevelComment.snippet.textDisplay for x in ct_by_video] + [self.description]
        comment_text.sort(key=self.fitness_comment_timestamps)
        yield from comment_text

    def get_songs_from_timestamps(self, s):
        song_lst = s.split('\n')
        start_seconds = []
        end_seconds = []
        song_title_lst = []
    ...
    def __call__(self, output, album_name=None, make_folder=True):
        if not self.timestamps_comment:
            self.timestamps_comment = next(self.comment_generator)
        if not self.songs:
            self.get_songs_from_timestamps(self.timestamps_comment)
        if not album_name:
            album_name = self.title
        # clearing temp folder just in case
        remove_temp_files()
        # download full audio from url with youtube-dl
        print('[SENDING LINK TO YOUTUBEDL] {}'.format(self.url))
        ...
        for song in self.songs:
            print('[slicing] song to {}/{}.webm'.format(output, song.song_name))
        ...
        # removing temp files again
        remove_temp_files()


def remove_temp_files():
    for i in os.listdir('{}/temp'.format(path)):
        print('[REMOVING FILE]' + i)
        os.remove(path+'/temp/'+i)
